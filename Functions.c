...
void sleep_func(char *a, int C, int R, int pos_equalto, int pos_end, int *arr, Graph *graph)
{
    ...
    // Check if the value inside SLEEP() is a number or a cell reference
    int sleep_value = -1; // Initialize with an invalid value
    int ref_cell = cell_parser(a, C, R, open_paren - a + 1, close_paren - a - 1, graph);

    if (ref_cell != -1)
    {
        // If a valid cell reference, fetch its value from arr
    ...
    {
        // Otherwise, parse it as a direct number
        sleep_value = strtol(open_paren + 1, &close_paren, 10);
        if (*close_paren != ')' || sleep_value <= 0)
        {
    ...
    // Execute sleep and update cell value
    sleep(sleep_value);
